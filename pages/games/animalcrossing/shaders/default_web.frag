...
uniform int u_pc_text_mode;
uniform vec4 u_pc_text_color;
...
void main() {
    ...
    if (u_pc_text_mode != 0) {
        ...
        vec2 outlineStep = texel * 0.24;
        float centerAlpha = texColor[0].a;
        float outlineAlpha = centerAlpha;

        outlineAlpha = max(outlineAlpha, texture(u_texture0, stc[0] + vec2( outlineStep.x, 0.0)).a);
        outlineAlpha = max(outlineAlpha, texture(u_texture0, stc[0] + vec2(-outlineStep.x, 0.0)).a);
        outlineAlpha = max(outlineAlpha, texture(u_texture0, stc[0] + vec2(0.0,  outlineStep.y)).a);
        outlineAlpha = max(outlineAlpha, texture(u_texture0, stc[0] + vec2(0.0, -outlineStep.y)).a);
        float fillAlpha = smoothstep(0.24, 0.62, centerAlpha);
        float maskAlpha = smoothstep(0.42, 0.80, outlineAlpha);
        float edgeAlpha = max(maskAlpha - fillAlpha, 0.0) * 0.22;
        float outAlpha = u_pc_text_color.a * max(fillAlpha, edgeAlpha);
        vec3 outColor = mix(vec3(0.0), u_pc_text_color.rgb, fillAlpha);
